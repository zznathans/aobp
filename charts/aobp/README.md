...
| mongodb.externalSecret.remoteKey | string | `""` | Key/path within the external store holding the connection details. The remote secret is expected to be a JSON object with `db_host`, `db_port`, `db_user`, `db_password`, and `db_name` keys - the shape DigitalOcean Managed MongoDB writes - which are assembled into a connection string. |
